...
class GeoPath {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.poses.forEach((val) => {
      bufferOffset = GeoPoseStamped.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.poses[i] = GeoPoseStamped.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.poses.forEach((val) => {
      length += GeoPoseStamped.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new GeoPath(null);
    ...
    if (msg.poses !== undefined) {
      ...
      for (let i = 0; i < resolved.poses.length; ++i) {
        resolved.poses[i] = GeoPoseStamped.Resolve(msg.poses[i]);
      }
    }
    else {
      resolved.poses = []
